...
| 2 | Prove `eigenvalue_reliability` (Theorem 6.4) | Apply Lemma 5.6, split `u - u*ₕ` at the conforming average `ũₕ`, use Gauss' theorem on `(div(σ-σₕ), u-ũₕ)`, then Theorem 6.2 (`reliability`), (67) and Young's inequality. Needs a uniform bound on `κ*ₕ` as an extra hypothesis. |
