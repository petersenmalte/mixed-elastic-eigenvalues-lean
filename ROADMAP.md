...
| Step | Content | Notes |
|---|---|---|
| 1 | Prove `cea_estimate` (Theorem 3.1) and then `uniform_convergence` (Theorem 4.7) | Brezzi-type estimates with `Metric.infDist`; existence via finite dimensionality. |
| 2 | Prove `reliability` (Theorem 6.2) and `eigenvalue_reliability` (Theorem 6.4) | Lemma 6.1 first; the absorption argument of `postprocessed_eigenvalue_rate` can be reused for Theorem 6.4. |
