...
theorem residual_le_estimator (i : ι) :
    D.h i * ‖PP.κstar i • PP.ustar i + div (E.q i).σₕ‖ ≤ A.estimator i :=
  A.vol_le_estimator i

/-- Both bounds for the conforming average, with the scaled `L²` estimate first. -/
theorem average_error_le_estimator (i : ι) :
    ...
    ‖PP.grad (PP.ustar i) - PP.grad (A.avg i (PP.ustar i))‖
      ≤ Real.sqrt A.Cavg * A.estimator i := by
  constructor
  · simpa only [mul_assoc] using A.avg_le i
  · exact A.grad_avg_le i
